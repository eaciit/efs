...
func parseFormulaSub(formulaTxt string, fisubs []*Formula) (*Formula, error) {
	...
	for i := 0; i < txtLen; i++ {
		...
		if c == "(" && !inBracket {
			...
			if efi != nil {
				...
				return nil, errors.New(toolkit.Sprintf("parseFormula: %s unable to parse it into *Formula", tmp))
			}
			fisubs = append(fisubs, fi)
			parts = append(parts, toolkit.Sprintf("@b_%d", len(fisubs)-1))
			inBracket = false
			tmp = ""
		} else if i == txtLen-1 {
			tmp += c
	...
	if len(fparts) == 1 {
		fpart := fparts[0]
		multiVariable := false
		if splis, _ := toolkit.Split(fpart, signList); len(splis) > 1 {
			multiVariable = true
		}
		ret.BaseOp = "+"
		if isMultiply(fpart) {
		...
		for fpartidx := fpartlen - 1; fpartidx >= 0; fpartidx-- {
			...
			if ((c == "*" || c == "/" || c == "^") && ret.BaseOp == "*") ||
				...
				if strings.Contains(tmp, "@b_") {
					...
					formulaIndex := int(-1)
					formulaIndex = toolkit.ToInt(tmp[3:], toolkit.RoundingAuto)
					if formulaIndex >= len(fisubs) {
						return nil, errors.New(toolkit.Sprintf("parseFormula: %s Subformula index-%d is not available", tmp, formulaIndex))
					}
...
func (f *Formula) Run(in toolkit.M) float64 {
	...
	if len(f.SubFormulas) == 0 {
		...
		for idx, sf := range f.SubFormulas {
			...
			lastvalue = v
			dbg += toolkit.Sprintf("%d=%.2f ", idx, v)
		}
	}
